...
def save_entry():
    ...
    Receives data from the journal entry form.
    """
    # Extract form data
    entry_title = request.form['title']
    entry_content = request.form['content']
    entry_date = request.form.get('date', datetime.now().strftime('%Y-%m-%d'))
    
    # Save to database
    with sqlite3.connect('journal.db') as conn:
        cursor = conn.cursor()
        # First check for existing entry with exact date match
        cursor.execute("SELECT id FROM journal WHERE date = ?", (entry_date,))
        ...
        if existing_entry:
            ...
            cursor.execute("""
                ...
                SET title = ?, content = ? 
                WHERE id = ?""", 
                (entry_title, entry_content, existing_entry[0]))
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?)""", 
                (entry_title, entry_content, entry_date))
